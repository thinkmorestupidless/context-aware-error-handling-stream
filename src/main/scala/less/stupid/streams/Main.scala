...
import akka.stream.scaladsl.{Flow, FlowWithContext, GraphDSL, Partition, RestartFlow, RunnableGraph, Sink, Source}
import akka.stream.{ActorAttributes, FlowShape, Materializer, RestartSettings, Supervision}
...
import less.stupid.streams.domain.Decoder.{DecodingError, DecodingException, UnableToDecodeBecauseItsHardcodedToFail}
import less.stupid.streams.domain.Service.{ServiceCallFailedBecauseSomeWeirdThingHappened, ServiceError, ServiceException}
import less.stupid.streams.domain._
import less.stupid.streams.infrastructure.{SometimesFailingDecoder, SometimesFailingService, SuccessfulDecoder, SuccessfulService}
...
//    val service = new SuccessfulService
    ...
    val processingFlow: KinesisProcessingFlow[DomainError, EncodedMessage] = FlowWithContext[EncodedMessage, KinesisContext].mapAsync(1)(decoder.decode)
      .mapAsync(1) {
...
object aws {
  ...
  object domain {
    ...
    type KinesisProcessingFlow[Error, In] = FlowWithContext[In, KinesisContext, Either[Error, Unit], KinesisContext, NotUsed]

    trait OffsetWriter {
      def writeOffsetForContext[Error](errorOrMessage: Either[Error, Unit], context: KinesisContext): Future[Either[OffsetWriterError, Unit]]
    }

    object OffsetWriter {
      sealed trait OffsetWriterError
      class OffsetWriterException(offsetWriterError: OffsetWriterError) extends RuntimeException(s"Error writing offset [$offsetWriterError]")
    }
  }
  ...
  object infrastructure {
    ...
    class SuccessfulOffsetWriter extends OffsetWriter {
      ...
      override def writeOffsetForContext[Error](errorOr: Either[Error, Unit], context: domain.KinesisContext): Future[Either[OffsetWriterError, Unit]] = {
        errorOr match {
    ...
    object KinesisOffsetWritingFlow {
      ...
      def apply[Error](offsetWriter: OffsetWriter, restartSettings: RestartSettings)(implicit ec: ExecutionContext): KinesisOffsetWritingFlow[Error] =
        RestartFlow.onFailuresWithBackoff(restartSettings) { () =>
          Flow[(Either[Error, Unit], KinesisContext)].mapAsync(1) {
            case (errorOrMessage, context) => offsetWriter.writeOffsetForContext(errorOrMessage, context).map {
    ...
    object KinesisStream {
      ...
      def apply[Error, In](source: Source[In, NotUsed], contextProvider: In => KinesisContext, processingFlow: KinesisProcessingFlow[Error, In], offsetWriter: OffsetWriter, restartSettings: RestartSettings)(implicit ec: ExecutionContext, mat: Materializer): RunnableGraph[NotUsed] = {
        val offsetWritingFlow = KinesisOffsetWritingFlow[Error](offsetWriter, restartSettings)
        source.asSourceWithContext(contextProvider)
          .via(processingFlow)
          .asSource
